...
public class InterfazUsuario {
    ...
    public void empezar() {
        try {
            System.out.println("Cargando datos de disco");
            controlador.carga(NOMBRE_ARCHIVO);
        } catch (IOException e) {
            System.out.println("Parece que es la primera vez que ejecutas este programa.");
            System.out.println("El archivo " + NOMBRE_ARCHIVO + " se creará al salir.");
        }
        String comando = null;
        do {
            try {
        ...
        try {
            System.out.println("Guardando datos disco");
            controlador.guarda(NOMBRE_ARCHIVO);
        } catch (IOException e) {
            // Esto no debería suceder!! Pero mostramos el stacktrace para que, si pasara,
            // pudiéramos analizar por qué y dónde ha sucedido
            e.printStackTrace();
        }
    }
